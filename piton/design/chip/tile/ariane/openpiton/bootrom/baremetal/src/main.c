...
int main() {
  ...
  if (whoami() == 1) {
    while (1) {
      if (readFromAddress(dma, DMA_END)== 1)
        return 0;
      while (readFromAddress(dma, DMA_START)==0) {
        if (readFromAddress(dma, DMA_END)== 1)
          asm volatile ("call _hang"); 
       do_delay(15);  
      }
      //printf("This is hart %d\n", whoami()); 
      writeToAddress(dma, DMA_LOCK, 4);   
      while (readFromAddress(dma, DMA_LOCK) != 4){
        do_delay(5); 
        writeToAddress(dma, DMA_LOCK, 4);   
      }
      writeToAddress(dma, DMA_START, 0);   
      while(1) {
        valid = readFromAddress(dma, DMA_VALID); 
        if(valid && DMA_DONE_MASK) 
            break; 
        do_delay(5); 
      } 
      src_addr = readFromAddress(dma, DMA_SRC_ADDR_MSB); 
      src_addr = ( src_addr << 32 ) | readFromAddress(dma, DMA_SRC_ADDR_LSB); 
      dst_addr = readFromAddress(dma, DMA_DST_ADDR_MSB); 
      dst_addr = ( dst_addr << 32 ) | readFromAddress(dma, DMA_DST_ADDR_LSB); 
      len = readFromAddress(dma, DMA_LENGTH); 
      if (valid && DMA_READ_MASK)
        memcpy_w(buf, src_addr, len);   
      if (valid && DMA_WRITE_MASK)
        memcpy_w(dst_addr, buf, len); 
      writeToAddress(dma, DMA_DONE, 1);   
